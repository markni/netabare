<template>
  <div v-if="loaded" class="h-full sm:pb-32">
    ...
    <div class="flex justify-end sm:pr-10 sm:pt-10 text-xl items-center gap-4">
      <span>筛选：从</span>
      <input v-model="startingYear"
             class="bg-transparent border-b border-gray-800 dark:border-gray-100 w-[80px] text-fie" max="2050"
             min="1900"
             type="number"/>
      <span>至</span>
      <input
          v-model="endingYear"
          class="bg-transparent border-b w-[80px] border-gray-800 dark:border-gray-100 text-fie" max="2050" min="1900"
          type="number"/>
      <span>且高于</span>
      <input v-model="minScore" class="bg-transparent border-gray-800 dark:border-gray-100 border-b w-[40px] text-fie"
             max="0" min="9"
             type="number"/>
      <span>分</span>
    </div>
